...
export class SearchBarComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
	...
	*/
	private searchSuggestionsAPIRequest() {
		this.queryField.valueChanges
			.pipe(
				debounceTime(1000),
				distinctUntilChanged(),
				switchMap(input => {
					if (this.showDropdown || this.isTypeUPC) {
						return [];
					}
				...
				map(res => this.searchProducts$.next(res))
			)
			.subscribe();
	...
	*/
	private processTextInputSearchBar(input: any) {
		this.foods = [];
		...
		if (input.length >= 2) {
			...
			this.searchSuggestionsAPIRequest();
		} else {
			this.icon = false;
			this.hide = true;
			this.grocerySelected = false;
