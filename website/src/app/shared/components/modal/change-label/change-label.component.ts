...
export class ChangeLabelComponent implements OnInit, OnChanges, AfterViewInit {
	...
	private _getUserListLabels() {
		...
		this._subscriptions.add(this._listLabelService.getUserListLabels(userId, this.listId).subscribe((res: any) => {
			if (res) {
				this.listLabels = res;
				this.loadingData = false;
	...
	private _updateLabels(userId: number, listId: string, labels: any[]) {
		this._subscriptions.add(this._listLabelService.putUserListLabels(userId, listId, labels).subscribe((res: any) => {
			this.loading = false;
			if (res) {
				this.listLabels = res;
				this._listLabelService.data$.next(res);
