...
export class AddPaymentMethodComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private _createInitialPaymentMethod() {
		...
		tokenPromise.then((res: any) => {
			if (res.token) {
				...
				this._subscriptions.add(this._userService.createStripeAccount(obj).subscribe((res: any) => {
					this.loading = false;
					this.state.complete();
					this.state.stop();
	...
	private _createPaymentMethod() {
		...
		tokenPromise.then((res: any) => {
			if (res.token) {
				...
				this._subscriptions.add(this._userService.createPaymentMethod(userId, obj).subscribe((res: any) => {
					this.loading = false;
					this.state.complete();
					this.state.stop();
	...
	private _updateUserProfile() {
		...
		if (profileUpdated) {
			this._subscriptions.add(this._userService.updateProfile(userId, this.profile).subscribe((res: any) => {
				if (res.success) {
