...
export class EditPaymentMethodComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
	...
	updatePaymentMethod() {
		...
		this._subscriptions.add(this._userAPI.editPaymentMethod(userId, paymentId, obj).subscribe((res: any) => {
			if (res) {
	...
	private _updateUserProfile(profile: IProfile) {
		...
		if (profileUpdated) {
			this._subscriptions.add(this._userAPI.updateProfile(userId, profile).subscribe((res: any) => {
				if (res.success) {
